...
function func.saveChar(name, lastName, age, char, id, phone)
    local source = source
    ...
    if name and lastName and age then
        ...
        vRP.UpgradeNames(vRP.Passport(source), name, lastName)
...
function UpdateUserPhone(user_id, phone)
    vRP.Query("characters/updatePhone", { phone = phone, id = user_id })
end
...
function setPlayerTattoos(source, user_id)
    TriggerClientEvent("reloadtattos", source, getUserTattoos(user_id))
...
function getUserMaxChars(source)
    ...
    if vRP.steamPremium(steam) then
        amountCharacters = amountCharacters + 0
    end
...
function func.tryCreateChar()
    ...
    if #data < getUserMaxChars(source)  then -- limite de personagens
        vRP.Query("nation_creator/create_characters",{ steam = steam, name = "", name2 = "", phone = "", blood = math.random(4) })
