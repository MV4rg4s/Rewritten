...
-- REQUESTINVENTORY
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.requestInventory()
	local source = source
...
-- DROPSERVER
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.DropServer(Coords,Item,Amount)
	local Number = 0
...
-- DROPS
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.Drops(Item,Slot,Amount,x,y,z)
	local source = source
...
-- PICKUP
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.Pickup(Number,Amount,Slot)
	local source = source
	local Slot = tostring(Slot)
...
-- SENDITEM
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.SendItem(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
...
-- TRASH
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.Trash(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
	local Amount = parseInt(Amount)
	local Passport = vRP.Passport(source)
...
-- USEITEM
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.UseItem(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
	local Amount = parseInt(Amount)
	local Passport = vRP.Passport(source)
...
-- VERIFYWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.verifyWeapon(Item,Ammo)
	local source = source
	local Passport = vRP.Passport(source)
...
-- DROPWEAPONS
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.dropWeapons(Item)
	local source = source
	local Passport = vRP.Passport(source)
...
-- REMOVETHROWING
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.removeThrowing(Item)
	local source = source
	local Passport = vRP.Passport(source)
...
-- PREVENTWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.preventWeapon(Item,Ammo)
	local source = source
	local Passport = vRP.Passport(source)
...
-- VERIFYOBJECTS
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.VerifyObjects(Entity,Service)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
...
-- STEALTRUNK
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.StealTrunk(Entity)
	local source = source
	local Plate = Entity[1]
...
-- STOREOBJECTS
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.StoreObjects(Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
-- MAKEPRODUCTS
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.MakeProducts(Table)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
...
-- DISMANTLE
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.Dismantle(Entity)
	local source = source
...
-- REMOVETYRES
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.RemoveTyres(Entity)
	local source = source
	local Passport = vRP.Passport(source)
...
-- REGISTERSTIMERS
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.RegistersTimers(Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
