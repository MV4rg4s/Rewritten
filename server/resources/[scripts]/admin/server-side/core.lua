...
-----------------------------------------------------------------------------------------------------------------------------------------
ghost = {}
Tunnel.bindInterface("admin",ghost)
...
RegisterCommand("blips",function(source)
	local Passport = vRP.Passport(source)
	if Passport then
		if vRP.HasGroup(Passport,"Admin",1) or vRP.HasGroup(Passport,"Admin",2) or vRP.HasGroup(Passport,"Staff",1)  then
			local Text = ""
	...
	if Passport then
		if vRP.HasGroup(Passport,"Admin",1) or vRP.HasGroup(Passport,"Admin",2) or vRP.HasGroup(Passport,"Staff",1) then
			...
			if not Noclip[Passport] then
				...
				Text = "Ativado"
				TriggerClientEvent("Notify",source,"verde","Noclip ligado.",5000)
			else
				Noclip[Passport] = nil
				Text = "Desativado"
				TriggerClientEvent("Notify",source,"vermelho","Noclip desligado.",5000)
			end
...
-----------------------------------------------------------------------------------------------------------------------------------------
	RegisterCommand("fixar",function(source)
		local Passport = vRP.Passport(source)
		if Passport then
			if vRP.HasGroup(Passport,"Admin",1) or vRP.HasGroup(Passport,"Admin",2) then
...
-----------------------------------------------------------------------------------------------------------------------------------------
local Checkpoint = 0
function ghost.raceCoords(vehCoords,leftCoords,rightCoords)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
