...
-----------------------------------------------------------------------------------------------------------------------------------------
ghost = {}
Tunnel.bindInterface("garages",ghost)
...
-- SERVERVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.ServerVehicle(Model,x,y,z,Heading,Plate,Nitrox,Doors,Body,Fuel)
...
-- VEHICLES
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.Vehicles(Number)
	local source = source
	local Passport = vRP.Passport(source)
...
function ghost.Spawn(Name, Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
					...
					if Coords then
						...
						if Garages[Number]["payment"] then
							if vRP.UserPremium(Passport) then
								TriggerClientEvent("dynamic:closeSystem",source)
								local Exist,Network = ghost.ServerVehicle(Name,Coords[1],Coords[2],Coords[3],Coords[4],Plate,vehicle[1]["nitro"],vehicle[1]["doors"],vehicle[1]["body"])
...
RegisterCommand("car",function(source,Message)
	local Passport = vRP.Passport(source)
	if Passport then
		if vRP.HasGroup(Passport,"Admin",1) or vRP.HasGroup(Passport,"Admin",2) and Message[1] then
			local VehicleName = Message[1]
			local Ped = GetPlayerPed(source)
			local Coords = GetEntityCoords(Ped)
			local Heading = GetEntityHeading(Ped)
			local Plate = "VEH"..(10000 + Passport)
			local Exist,Network,Vehicle = ghost.ServerVehicle(VehicleName,Coords["x"],Coords["y"],Coords["z"],Heading,Plate,2000,nil,1000)
...
-- DELETE
-----------------------------------------------------------------------------------------------------------------------------------------
function ghost.Delete(Network,Health,Engine,Body,Fuel,Doors,Windows,Tyres,Plate)
	if Spawn[Plate] then
