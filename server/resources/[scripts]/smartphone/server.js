...
if (pB.base === 'creative_v3') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (c, d) => {
    ...
    if (pB.base === 'creative_v4') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (d, e) => {
        ...
        } else pB.base === 'ghost_network' && (pB.serverSideProp = true, pp.getUserSource = pp.Source, pp.getUserId = pp.Passport, pp.getInventoryItemAmount = pp.InventoryItemAmount, pp.hasPermission = pp.HasPermission, pp.request = pp.Request, pp.getBankMoney = async c => {
...
(pB.base === 'creative_v4' || pB.base === 'new_ghost') && (pp.getUserSource = pp.userSource, pp.getUsersByPermission = async c => {
    const d = [];
...
const pN = {
    'new_ghost': 'checkBroken',
    'ghost_network': 'CheckDamaged'
};
const pO = pN;
    ...
    try {
        while (true) {
            ...
            return await pF.fetchTables(), await Promise.resolve().then(function () {
                ...
                return s9;
            }), emit('smartphone:isReady'), console.log('[SMARTPHONE] CELULAR AUTENTICADO COM SUCESSO!'), pz.metadata('video_server', config.client.videoServer || null), pz.metadata('apps', config.client.customApps || null);
        }
    } catch (x) {
